...
import { Plus, Orbit, BarChart3, CalendarDays, Sparkles, Search, SlidersHorizontal, X, Edit3, Trash2, Share2, Download, Moon, Sun, ChevronLeft, MoreVertical, Save, Heart, Home, Briefcase, Users, Coins, Star, RefreshCcw, ImagePlus, Images } from 'lucide-react';
...
const makeId=()=> (typeof crypto!=='undefined'&&crypto.randomUUID?makeId():`id-${Date.now()}-${Math.random().toString(16).slice(2)}`);
...
function Detail({item,onClose,onDelete,onSave}){const e=emotionOf(item.emotion);const cardRef=useRef(null);const exportImg=async()=>{if(!cardRef.current)return;const canvas=await html2canvas(cardRef.current,{backgroundColor:null,scale:2});const a=document.createElement('a');a.href=canvas.toDataURL();a.download=`emotion-memory-${item.date}.png`;a.click()};return <motion.div className="overlay" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}}><motion.div className="detail" initial={{y:70}} animate={{y:0}} exit={{y:80}} ref={cardRef}><div className="detail-top"><button onClick={onClose}><ChevronLeft/></button><button><MoreVertical/></button></div><div className="big-ball" style={bgStyle(e)}><span>{e.face}</span><b>{item.title}</b><i>♥</i></div><div className="detail-card"><div className="between"><div><h3>{e.zh}</h3><p>{e.en}</p></div><p>{item.date}</p></div><p className="content">{item.content}</p>{item.images?.length>0&&<div className="detail-images"><div className="image-title"><Images size={16}/> 相關圖片</div>{item.images.map((img,idx)=><a key={img.id||idx} href={img.data} target="_blank" rel="noreferrer"><img src={img.data} alt={img.name||`memory-${idx+1}`}/></a>)}</div>}<div className="stars">{'★'.repeat(item.intensity)}{'☆'.repeat(10-item.intensity)} <span>{item.intensity}/10</span></div><div className="chips">{(item.categories||[]).map(c=><span className="chip active" key={c}>{c}</span>)}</div></div><div className="actions"><button><Edit3/>編輯</button><button onClick={exportImg}><Download/>匯出</button><button onClick={()=>onDelete(item.id)} className="danger"><Trash2/>刪除</button></div></motion.div></motion.div>}
